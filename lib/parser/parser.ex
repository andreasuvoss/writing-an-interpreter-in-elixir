defmodule Parser do
  ...
  def parse_program(tokens) do
    case parse_statements(tokens) do
      {:ok, statements} -> {:ok, %AST.Program{statements: statements}}
  ...
  defp parse_statements(tokens, acc \\ [], errors \\ []) do
    case parse_statement(tokens) do
      ...
      {:ok, nil, remaining_tokens} ->
        parse_statements(remaining_tokens, acc, errors)

      {:ok, stmt, remaining_tokens} ->
        parse_statements(remaining_tokens, [stmt | acc], errors)

      {:error, errs, remaining_tokens} ->
        parse_statements(remaining_tokens, acc, errs ++ errors)
    end
  end
  ...
  defp parse_expression_statement([head | tail]) do
    case parse_expression([head | tail]) do
      {:ok, expr, [%Token{type: :semicolon} | tl]} ->
        {:ok, %AST.ExpressionStatement{expression: expr}, tl}
  ...
  defp parse_let_statement([%Token{type: :eof} | tail]),
    do: {:error, ["expected identifier after let statement"], tail}

  defp parse_let_statement([%Token{type: :ident} = token | tail]),
    do: {:error, ["expected '=' after let #{token.literal}"], tail}

  defp parse_let_statement([token | tail]),
    do: {:error, ["unexpected symbol '#{token.literal}' in let statement"], tail}
    ...
    case parse_expression(tail, :lowest) do
      {:ok, expr, [peek_token | rest]} ->
        if peek_token.type != :rparen do
          {:error,
           ["expected ')' to close grouped expression got '#{peek_token.literal}' instead"], rest}
        else
          {:ok, expr, rest}
        end

      {:ok, _, []} ->
        {:error, ["unclosed grouped expression"], []}
  ...
  defp parse_block_statement([%Token{} = token | tail], acc, errors) do
    case token.type do
      ...
      :rbrace ->
        case errors do
          [] -> {:ok, %AST.BlockStatement{statements: Enum.reverse(acc)}, tail}
          _ -> {:error, errors, tail}
        end

      :eof ->
        {:error, ["unterminated block statement"], tail}
  ...
  defp parse_block_statement([%Token{type: :eof}], _, errors),
    do: {:error, ["unterminated block statement" | errors], []}
      ...
      case parse_expression(rest, :lowest) do
        {:ok, condition, [%Token{type: :rparen}, %Token{type: :lbrace} = peek_token | tail]} ->
          case parse_block_statement([peek_token | tail]) do
            ...
            {:ok, consequence, tail} ->
              {:ok,
               %AST.IfExpression{token: token, condition: condition, consequence: consequence},
               tail}
        ...
        {:ok, condition, [_ | tail]} ->
          {:error, ["missing block for condition #{condition}"], tail}
      end
    end
  end
      ...
      case parse_function_parameters(rest) do
        {:ok, params, rest} ->
          case parse_block_statement(rest) do
            {:ok, statements, rest} ->
              {:ok, %AST.MacroLiteral{parameters: params, body: statements}, rest}
    ...
    if token.type != :lparen do
      ...
      case parse_function_parameters(rest) do
        {:ok, params, rest} ->
          case parse_block_statement(rest) do
            {:ok, statements, rest} ->
              {:ok, %AST.FunctionLiteral{parameters: params, body: statements}, rest}
  ...
  defp parse_function_parameters([token | tokens], acc) do
    case token.type do
      ...
      _ ->
        {:error, ["unexpected symbol '#{token.literal}' (#{token.type}) in function parameters"],
         tokens}
    end
  end

  defp parse_function_parameters([], _),
    do: {:error, ["could not parse function parameters for function literal"], []}
  ...
  defp parse_call_expression([token | rest], function) do
    if token.type != :lparen do
      ...
      case parse_call_arguments(rest) do
        {:ok, args, rest} ->
          {:ok, %AST.CallExpression{token: token, function: function, arguments: args}, rest}
  ...
  defp parse_call_arguments([token | tokens], acc, end_token) do
    case token.type do
      ...
      :comma ->
        parse_call_arguments(tokens, acc, end_token)
  ...
  defp parse_call_arguments([], _, _),
    do: {:error, ["could not parse call arguments for call expression"], []}

  defp parse_string_literal([%Token{type: :string} = token | tail]) do
    {:ok, %AST.StringLiteral{token: token, value: token.literal}, tail}
  end

  defp parse_array_literal(tokens) do
    case parse_call_arguments(tokens, [], :rbracket) do
      {:ok, elements, tokens} -> {:ok, %AST.ArrayLiteral{elements: elements}, tokens}
    end
  end

  defp parse_hash_literal([token | tail] = tokens, hash \\ %{}) do
    case token.type do
      :rbrace -> {:ok, %AST.HashLiteral{pairs: hash}, tail}
      :eof -> {:ok, %AST.HashLiteral{pairs: hash}, []}
      _ ->
        case parse_expression(tokens, :lowest) do
          {:ok, key, [%Token{type: :colon} | tail]} ->
            case parse_expression(tail, :lowest) do
          ...
          {:ok, _, [next_token | tail]} ->
            {:error, ["unexpected token after hash key got #{next_token.literal} expected :"], tail}
          {:error, _, tail} -> {:error, ["unable to parse hash literal"], tail}
        end
    end
  end
  ...
  defp parse_prefix([%Token{type: :rbrace} | tail]),
    do: {:error, ["found '}' without a block to close"], tail}

  defp parse_prefix([%Token{type: :else} | tail]),
    do: {:error, ["found else keyword with no prior if block"], tail}

  defp parse_prefix([%Token{type: :assign} | tail]),
    do: {:error, ["assignment '=' without let statement is not allowed"], tail}

  defp parse_prefix([%Token{literal: literal, type: type} | tail]),
    do:
      {:error,
       ["no prefix parse function for #{String.upcase(Atom.to_string(type))}: #{literal}"], tail}
  ...
  defp parse_infix_expression(left, [%Token{} = token | tail] = rest, precedence) do
    ...
    if current_precedence > precedence and infix_operator?(token) and token.type != :semicolon do
      case token.type do
        :lparen ->
          case parse_call_expression([token | tail], left) do
            {:ok, call, tail} -> parse_infix_expression(call, tail, precedence)
            {:error, errors, rest} -> {:error, errors, rest}
          end
        :lbracket ->
          case parse_index_expression([token | tail], left) do
  ...
  defp parse_index_expression([token | tail], left) do
    case parse_expression(tail, :lowest) do
      {:ok, val, [%Token{type: :rbracket} | tail]} ->
        {:ok, %AST.IndexExpression{token: token, left: left, index: val}, tail}
